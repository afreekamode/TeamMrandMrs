...
<?php 
	...
	$check_pic = mysqli_query($conn, "SELECT cover_pic FROM users WHERE username='$username'");
	...
	if (@($_POST['follow'])) {
		$if_followed = mysqli_query($conn, "SELECT * FROM follow WHERE user_to='$username' AND user_from='$user'");
		...
		$user_from = $user;
		$user_to = $username;
		$create_follow = mysqli_query($conn, "INSERT INTO follow VALUES ('', '$user_from', '$user_to', NOW(), 'no')");
		}
	}

	//unfollow request system
	if (@($_POST['unfollow'])) {
		$if_following = mysqli_query($conn, "SELECT * FROM follow WHERE user_to='$username' AND user_from='$user'");
		$following_found = mysqli_num_rows($if_following);
		if ( $following_found >= 1 ) {
			$delete_follow = mysqli_query($conn, "DELETE FROM follow WHERE user_from='$user' && user_to='$username' ");
		}else{
			header("location: profile.php?u=$username");
		}
	}
	...
	if (@($_POST['unfriend'])) {
		...
		$following_found = mysqli_num_rows($if_following);
		if ( $following_found >= 1 ) {
			$delete_follow1 = mysqli_query($conn, "DELETE FROM follow WHERE user_from='$user' && user_to='$username' ");
			$delete_follow2 = mysqli_query($conn, "DELETE FROM follow WHERE user_from='$username' && user_to='$user' ");
		}else{
			header("location: profile.php?u=$username");
		}
	}
	...
	$about_query = mysqli_query($conn, "SELECT first_name,verify_id FROM users WHERE username='$username'");
			...
			<li style="float: left; padding: 8px 0;">
				<form action="" method="POST">
					...
					if ($user == $username) {
						...
						$check_if_follow = mysqli_query($conn, "SELECT * FROM follow WHERE (user_from='$user' AND user_to='$username') ORDER BY id DESC LIMIT 2");
						$num_follow_found = mysqli_num_rows($check_if_follow);
						if ( $num_follow_found != "" ) {
							$check_if_friend = mysqli_query($conn, "SELECT * FROM follow WHERE (user_from='$username' AND user_to='$user') ORDER BY id DESC LIMIT 2");
							...
							$if_uname_follow = mysqli_query($conn, "SELECT * FROM follow WHERE (user_from='$username' AND user_to='$user') ORDER BY id DESC LIMIT 2");
