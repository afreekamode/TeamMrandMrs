...
$get_user_gender = mysqli_query($conn, "SELECT * FROM users WHERE username='$user'");
$gender_user_row = mysqli_fetch_assoc($get_user_gender);
$user_gender_value = $gender_user_row['gender'];
if($user_gender_value == 1) {
	$get_ppl_info = mysqli_query($conn, "SELECT * FROM users WHERE username!='$user' AND gender='1' AND activated='1' AND blocked_user='0' ORDER BY  RAND() ");
	//declear variable
	$getuserNum= '0';
	...
	$get_user = mysqli_query($conn, "SELECT * FROM users WHERE gender='2'");
