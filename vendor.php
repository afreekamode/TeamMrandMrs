...
if (isset($_POST['login'])) {
		if (isset($_POST['user_login']) && isset($_POST['password_login'])) {
			...
			$sql = "SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND activated='1' AND blocked_user='0'";
			...
			if ($num>0) {
				...
				$sql1 = "SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND activated ='0'";
				...
				if ($num1>0) {
					...
					$sql2 = "SELECT * FROM users WHERE (username='$user_login' || email='$user_login') AND password='$password_login_md5' AND blocked_user='1'";
...
if (isset($_POST['submconfrmCode'])) {
	$user_confrmCode_db = mysqli_real_escape_string($conn, $_POST['confrmCode']);
	...
	if ($num2>=1) {
		$pupdate_query = "UPDATE users SET activated='1', confirmCode='0' WHERE username='$user_loginnn'";
		$result3 = mysqli_query($conn, $pupdate_query);
...
if(isset($_POST["name2check"]) && $_POST["name2check"] != ""){
    ...
    if ($uname_check < 1) {
	    echo '<p style="color: #fff; font-size: 13px; font-weight: 600; text-align: center; margin: 3px 0;">'.$username .' is available!</p>';
	    exit();
    } else {
	    echo '<p style="color: #C10000; font-size: 13px; font-weight: 600; text-align: center; margin: 3px 0;"><strong>' . $username . '</strong> has been taken! Choose another username.</p>';
	    exit();
    }
}
?>
...
$_POST['username'] = preg_replace('/\s+/','',$_POST['username']);
	try {
		...
		$sql = "SELECT email FROM users WHERE email='$u_email'";
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >4 ) {
						...
						$sql = "INSERT INTO users (first_name,username,email,password,gender,sign_up_date,activated) VALUES ('$_POST[first_name]','$_POST[username]','$_POST[email]','$_POST[password]','$_POST[gender]','$d','1')";
						$unmail_check = mysqli_query($conn, $sql);
						...
						<div class="maincontent_text" style="text-align: center;">
						<font face="bookman">You can now login with your usename or email. <br>
							Email: '.$u_email.'<br>
							Username: '.$_POST['username'].'
						</font></div>';
						//}else {
...
$sql = "SELECT * FROM daowat WHERE photos != '' ORDER BY  RAND() ";
...
<html>
	<head>
	<title>Owanbe | Lagos</title>
	<meta charset="uft-8">
	<link rel="icon" href="./img/favicon.png" type="image/x-icon">
	<link rel="stylesheet" type="text/css" href="./css/style.css">
	<meta name="description" content="Hi there. Join Owanbe. Connect with your family and friends. Lagos to the World. Get update over the world." /><!--formatted-->
	    <meta name="viewport" content="width=device-width, initial-scale=1.0">
	    <meta name="author" content="Digitizer World">
		...
		<div class="main">
			<div class="headerLogin">
				<div class="login_menubar clearfix">
					<div class="menu_logo">
						<h1>
							<a title="Go to Owanbe Home" href="index.php">
								<b>Owanbe</b>
							</a>
						</h1>
					</div>
							...
							<div class="maincontent">
							   <?php
								if (isset($success_message)) {
									...
									echo '
									   <h2><font face="bookman">Hi and Welcome!</font></h2>
										<div class="maincontent_text">
										<font face="bookman">Join Owanbe.com <br>
											<li>Connect with your family and friends.</li>
											<li>Lagos to the World.</li>
											<li>Get update over the events and style close to you and around the world.</li>
										</font>
										</div>
									';
								}
